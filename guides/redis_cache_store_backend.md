...
defmodule MyAppWeb.Pow.RedisCacheTest do
  ...
  describe "with redis errors" do
    ...
    test "logs error" do
      assert CaptureLog.capture_log(fn ->
        RedisCache.put(@default_config, {"key", "value"})
        :timer.sleep(100)
      end) =~ "(RuntimeError) Redix received unexpected response [%Redix.Error{message: \"OOM command not allowed when used memory > 'maxmemory'.\"}"
    end
  end
  ...
  test "records auto purge" do
    ...
    RedisCache.put(Keyword.put(@default_config, :ttl, 150), [{["namespace", "key4"], "4"}])
    :timer.sleep(50)
